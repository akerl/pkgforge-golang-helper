.PHONY: default local build release manual clean lint vet fmt test deps init update generate custom
...
local: $(BASE) deps $(GOX) custom generate fmt lint vet test
	cd $(BASE) && $(GOX) \
